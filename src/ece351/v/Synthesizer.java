...
public final class Synthesizer extends PostOrderExprVisitor {
				...
				if (stmt instanceof Process) {
					for (Statement seqstmt : ((Process) stmt).sequentialStatements) {
						if (seqstmt instanceof IfElseStatement) {
							FProgram newlines = implication((IfElseStatement) seqstmt);
							result = result.appendAll(newlines);
						} else if (seqstmt instanceof AssignmentStatement) {
							AssignmentStatement toAdd = new AssignmentStatement(
									(VarExpr) traverseExpr(((AssignmentStatement) seqstmt).outputVar),
									traverseExpr(((AssignmentStatement) seqstmt).expr));
							result = result.append(toAdd);
						}
					}
				} else if (stmt instanceof IfElseStatement) {
					FProgram newlines = implication((IfElseStatement) stmt);
					result = result.appendAll(newlines);
				} else if (stmt instanceof AssignmentStatement) {
					AssignmentStatement toAdd = new AssignmentStatement(
							(VarExpr) traverseExpr(((AssignmentStatement) stmt).outputVar),
	...
	private FProgram implication(final IfElseStatement statement) {
		...
		// build result
		// TODO: longer code snippet
		ImmutableList<AssignmentStatement> newAssignmentStatements = ImmutableList.of();
		condCount++;
		VarExpr condition = new VarExpr(String.format("%s%d", conditionPrefix, condCount));
		AssignmentStatement conditionAssign = new AssignmentStatement(condition, traverseExpr(statement.condition));
		newAssignmentStatements = newAssignmentStatements.append(conditionAssign);
		AndExpr ifSide = new AndExpr(traverseExpr(ifb.expr), condition);
		AndExpr elseSide = new AndExpr(new NotExpr(condition), traverseExpr(elb.expr));

		AssignmentStatement assignment = new AssignmentStatement((VarExpr) traverseExpr(ifb.outputVar),
				new OrExpr(ifSide, elseSide));

		newAssignmentStatements = newAssignmentStatements.append(assignment);

		return new FProgram(newAssignmentStatements);
	}
	...
	public Expr visitXOr(XOrExpr e) {
		throw new IllegalStateException("xor not desugared");
	}

	@Override
	public Expr visitEqual(EqualExpr e) {
		throw new IllegalStateException("EqualExpr not desugared");
	}

	@Override
	public Expr visitNAnd(NAndExpr e) {
		throw new IllegalStateException("nand not desugared");
	}

	@Override
	public Expr visitNOr(NOrExpr e) {
		throw new IllegalStateException("nor not desugared");
	}

	@Override
	public Expr visitXNOr(XNOrExpr e) {
		throw new IllegalStateException("xnor not desugared");
	}
